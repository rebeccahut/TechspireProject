-- Kyle Dela Pena
-- Duration of employment of inactive employees
-- Report can be used for finding the correlation between inactive employees and their employee classifications
-- Displays decased employees with their associations along with the date they stopped being an employee
-- row num, first name, last name, duration, job name, type name, category name, end date -ordered by duration, last name

SELECT ROW_NUMBER() 
OVER(ORDER BY Employee.last_name) AS num_row, Employee.first_name, Employee.last_name, DATEDIFF(day, begin_date, IIF(end_date IS NOT NULL , end_date, getdate() )) AS duration_days, Job.job_name, EmployeeType.type_name, EmployeeCategory.category_name, Employee.end_date 
from Employee
